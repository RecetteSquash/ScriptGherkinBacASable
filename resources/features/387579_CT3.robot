...
Test Setup
    ...
    ...                You can define the ${TEST_387579_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_387579_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =           Get Variable Value    ${TEST_SETUP}
    ${TEST_387579_SETUP_VALUE} =    Get Variable Value    ${TEST_387579_SETUP}
    IF    $TEST_SETUP_VALUE is not None
        Run Keyword    ${TEST_SETUP}
    END
    IF    $TEST_387579_SETUP_VALUE is not None
        Run Keyword    ${TEST_387579_SETUP}
    END

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_387579_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_387579_TEARDOWN}.

    ${TEST_387579_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_387579_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =           Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_387579_TEARDOWN_VALUE is not None
        Run Keyword    ${TEST_387579_TEARDOWN}
    END
