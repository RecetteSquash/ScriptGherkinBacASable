...
Test Setup
    ...
    ...                You can define the ${TEST_26336_SETUP} variable with a keyword for setting up this specific test.
    ...                If both are defined, ${TEST_26336_SETUP} will be run after ${TEST_SETUP}.

    ${TEST_SETUP_VALUE} =          Get Variable Value    ${TEST_SETUP}
    ${TEST_26336_SETUP_VALUE} =    Get Variable Value    ${TEST_26336_SETUP}
    IF    $TEST_SETUP_VALUE is not None
        Run Keyword    ${TEST_SETUP}
    END
    IF    $TEST_26336_SETUP_VALUE is not None
        Run Keyword    ${TEST_26336_SETUP}
    END

Test Teardown
    [Documentation]    test teardown
    ...                You can define the ${TEST_TEARDOWN} variable with a keyword for tearing down all your tests.
    ...                You can define the ${TEST_26336_TEARDOWN} variable with a keyword for tearing down this specific test.
    ...                If both are defined, ${TEST_TEARDOWN} will be run after ${TEST_26336_TEARDOWN}.

    ${TEST_26336_TEARDOWN_VALUE} =    Get Variable Value    ${TEST_26336_TEARDOWN}
    ${TEST_TEARDOWN_VALUE} =          Get Variable Value    ${TEST_TEARDOWN}
    IF    $TEST_26336_TEARDOWN_VALUE is not None
        Run Keyword    ${TEST_26336_TEARDOWN}
    END
